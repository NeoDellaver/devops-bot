#!/bin/bash
# 🔍 Скрипт диагностики devops-bot

echo "═════════════════════════════════════════════"
echo "🔍 ДИАГНОСТИКА DEVOPS-BOT"
echo "═════════════════════════════════════════════"
echo ""

cd /root/devops-bot

# Проверка 1: Статус сервиса
echo "📋 Статус сервиса:"
sudo systemctl status devops-bot.service --no-pager | head -20
echo ""

# Проверка 2: Последние ошибки
echo "❌ Последние ОШИБКИ в логах:"
sudo journalctl -u devops-bot.service -n 30 --no-pager | grep -E "ERROR|Exception|Traceback|CRITICAL" || echo "✅ Ошибок не найдено"
echo ""

# Проверка 3: Последние 50 строк логов
echo "📊 Последние 50 строк логов:"
sudo journalctl -u devops-bot.service -n 50 --no-pager
echo ""

# Проверка 4: Процесс Python
echo "🔄 Процесс bot.py:"
ps aux | grep -E "bot\.py|devops" | grep -v grep || echo "❌ Процесс не запущен"
echo ""

# Проверка 5: Проверка конфига
echo "🔐 Проверка config.py:"
if [ -f "config.py" ]; then
    echo "✅ config.py существует"
    # Проверить есть ли BOT_TOKEN
    if grep -q "BOT_TOKEN" config.py; then
        echo "✅ BOT_TOKEN найден"
    else
        echo "❌ BOT_TOKEN НЕ найден в config.py!"
    fi
else
    echo "❌ config.py не найден!"
fi
echo ""

# Проверка 6: Проверка database
echo "💾 Проверка database.py:"
if [ -f "database.py" ]; then
    echo "✅ database.py существует"
else
    echo "❌ database.py не найден!"
fi
echo ""

# Проверка 7: Проверка handlers
echo "📁 Проверка handlers:"
if [ -d "handlers" ]; then
    ls -la handlers/ | grep ".py"
else
    echo "❌ Папка handlers не найдена!"
fi
echo ""

# Проверка 8: Синтаксис Python
echo "✔️ Проверка синтаксиса bot.py:"
source venv/bin/activate 2>/dev/null
python3 -m py_compile bot.py 2>&1 && echo "✅ Синтаксис OK" || echo "❌ Ошибка синтаксиса!"
deactivate 2>/dev/null
echo ""

echo "═════════════════════════════════════════════"
echo "🆘 ДЛЯ ПЕРЕЗАГРУЗКИ ВЫПОЛНИТЕ:"
echo "sudo systemctl restart devops-bot.service"
echo "═════════════════════════════════════════════"
